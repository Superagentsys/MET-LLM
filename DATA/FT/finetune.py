...
def load_tokenizer_and_model(
    ...
    if peft_config is not None:
        ...
        # Auto-detect LoRA target modules for DeepSeek/LLaMA/GLM families
        try:
            def detect_lora_targets(m):
                import torch as _torch
                candidates = set()
                for name, module in m.named_modules():
                    base = name.split('.')[-1]
                    if isinstance(module, _torch.nn.Linear):
                        if base in ("q_proj","k_proj","v_proj","o_proj","gate_proj","up_proj","down_proj","query_key_value","W_pack","c_attn","c_proj"):
                            candidates.add(base)
                # Prefer q/k/v/o if present
                prefer = [t for t in ("q_proj","k_proj","v_proj","o_proj") if t in candidates]
                if prefer:
                    return prefer
                if "query_key_value" in candidates:
                    return ["query_key_value"]
                if candidates:
                    return sorted(list(candidates))
                return ["q_proj","k_proj","v_proj","o_proj"]
            def has_any_target(m, targets):
                names = [n.split('.')[-1] for n, _ in m.named_modules()]
                return any(t in names for t in targets)
            auto_targets = detect_lora_targets(model)
            cfg_targets = getattr(peft_config, 'target_modules', None)
            if not cfg_targets or not has_any_target(model, cfg_targets):
                peft_config.target_modules = auto_targets
        except Exception:
            pass
